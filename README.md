...
- Each log has its own starting and current moisture, shown on its timeline card. Waiting wood ranges from roughly 7–35% moisture, while already burning logs retain a little residual moisture. Drying spends wood and core heat; wetter logs take longer to ignite and consume fuel more slowly.
...
- The timeline displays each log's current phase, remaining wood and char, arrival time, coal heat, and recent ignition / shedding / ash events. Ash persists when a position receives another log. The timing of low flames, embers and cooling depends on fuel moisture, density, the heat of the core and the feed schedule.
