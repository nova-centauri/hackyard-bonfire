// Time-based motion keeps the wood and stones fixed while combustion rises around them.
...
export function updateStudyMotion(study) {
  ...
  const driftSpeed = study.config.fireVariant === 2 ? 1.12 : .85;
  ...
  for (let i = 0; i < positions.count; i++) {
    ...
    const brightness = fade(age) * (.83 + Math.sin(time * 4 + i) * .17);
  ...
  for (const streak of motion.streaks) {
    ...
    streak.material.opacity = fade(age) * .66;
  }
  // Low-amplitude light variation keeps the material detail readable.
  motion.lights.forEach(({ light, intensity }, index) => {
    light.intensity = intensity * (1 + Math.sin(time * 6.2 + index * 2) * .045 + Math.sin(time * 10.7 + 1.3) * .025);
  });
  motion.coalMaterial.emissiveIntensity = motion.coalEmission * (1 + Math.sin(time * 2.3) * .065);
