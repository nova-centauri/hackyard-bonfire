...
// missing or blocked store simply yields the defaults. The fire opens in focus
// mode: the menus are there for whoever goes looking, and only a viewer who
// explicitly left focus mode comes back to them.
...
// sound and volume only; the focus choice starts over.
const KEY = 'bonfire.preferences.v2', LEGACY_KEY = 'bonfire.preferences.v1';
const DEFAULTS = Object.freeze({ focus: true, sound: false, volume: .3 });
...
export function sanitizePreferences(raw) {
  ...
  return { focus: source.focus !== false, sound: source.sound === true, volume: clampVolume(Number(source.volume)) };
}
