...
// they were in focus mode, whether sound was on, how loud, whether the fire
// tends itself, and quiet lifetime counts for the fire. Storage is a
...
// Lifetime stats live on the same v2 record so they survive a refresh the
// same way the other prefs do; a missing stats object starts at zero.
const KEY = 'bonfire.preferences.v2', LEGACY_KEY = 'bonfire.preferences.v1';
export const STATS_DEFAULTS = Object.freeze({ fires: 0, pieces: 0, burnSeconds: 0, tended: 0, pops: 0 });
const DEFAULTS = Object.freeze({ focus: true, sound: false, volume: .3, autoFeed: true, stats: STATS_DEFAULTS });

const clampVolume = value => Number.isFinite(value) ? Math.min(1, Math.max(0, value)) : DEFAULTS.volume;
const count = (value, integer) => {
  const n = Number(value);
  if (!Number.isFinite(n) || n <= 0) return 0;
  return integer ? Math.min(Number.MAX_SAFE_INTEGER, Math.floor(n)) : n;
};

export function sanitizeStats(raw) {
  const source = raw && typeof raw === 'object' && !Array.isArray(raw) ? raw : {};
  return {
    fires: count(source.fires, true),
    pieces: count(source.pieces, true),
    burnSeconds: count(source.burnSeconds, false),
    tended: count(source.tended, true),
    pops: count(source.pops, true),
  };
}
...
export function loadPreferences(storage = globalThis.localStorage) {
  try {
    ...
    return { ...DEFAULTS, sound, volume, stats: { ...STATS_DEFAULTS } };
...
export function savePreferences(patch, storage = globalThis.localStorage) {
  ...
  const next = sanitizePreferences({ ...current, ...source, stats: { ...current.stats, ...source.stats } });
