...
export function mountBurnPanel(viewer, { onAutoFeed } = {}) {
    ...
    <section class="burn-panel" aria-label="Fire lifecycle" hidden>
      <div class="burn-toolbar">
        ...
        <div class="burn-actions" id="burn-actions">
          <div class="fuel-add-control"><label class="fuel-control">Fuel<select id="fuel-type" aria-label="Fuel to add"><option value="">Next waiting piece</option>${Object.entries(FUEL_TYPES).map(([id, type]) => `<option value="${id}">${type.label}</option>`).join('')}</select></label><button id="add-log" class="burn-button">＋ Add fuel</button></div>
          <button id="randomize-fire" class="burn-button" title="Reset with new fuel types, ages, moisture, and coal heat">↻ Randomize</button>
  ...
  function update(force = false) {
    ...
    document.querySelector('#core-heat-note').textContent = {
      ...
      Healthy: 'Useful heat · wood catches as it dries.',
      'Very hot': 'Strong heat · wood burns faster.',
    }[coreStatus];
    ...
    cards.forEach((card, i) => {
      ...
      card.querySelector('.fuel-detail').textContent = log.phase === 'queued' ? cycle.autoFeed ? `Added in ${formatTime(cycle.nextFeed + queuedDelay - cycle.time)}` : 'Whole wood · on hold' : log.phase === 'ash' ? 'Returned to the fire bed' : log.phase === 'cold' ? 'Waiting for heat' : `${Math.round((log.wood + log.char) * 100)}% fuel · ${formatTime(cycle.time - log.addedAt)} on the bed`;
      if (log.phase === 'queued') queuedDelay += cycle.feedInterval / Math.max(1, fuelType.burnRate);
      ...
      card.setAttribute('aria-label', `${fuelType.label} ${log.id}: ${PHASE_LABELS[log.phase]}, ${Math.round((log.wood + log.char) * 100)} percent fuel remaining${log.phase === 'ash' ? '' : `, ${moisture} percent moisture, ${condition.toLowerCase()}`}`);
    });
