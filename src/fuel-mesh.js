...
export function createSceneFuelMesh({ definition, fuelType = 'log', seed, mode, hybrid, rand = random(seed) }, { barkMat, endMat, exposedMat }) {
  ...
  const sideMaterial = fuelType === 'plank' ? exposedMaterial : hybrid ? burningMaterial(barkMat, burnUniforms) : barkMat;
  const endMaterial = hybrid ? burningMaterial(endMat, burnUniforms, true) : endMat;
  ...
  const chips = [], chipCount = fuelType === 'kindling' ? 9 : mode === 1 ? 10 : 45;
  ...
  const charChips = addDetail(mergeGeometries(chips), hybrid ? burningMaterial(chipMaterial, burnUniforms) : chipMaterial);
