...
test('a hearth fire counts the mouth bed, not the pit’s seven slots', () => {
  const home = createSceneCycle('home', 4);
  assert.equal(home.logs.length, 5);
  assert.equal(home.piecesPlaced, home.logs.filter(log => log.phase !== 'queued').length);
  const storage = memory();
  const stats = createFireStats({ storage, persistBurnEvery: 1e9 });
  stats.observeCycle(home);
  assert.equal(stats.snapshot().fires, 1);
  assert.equal(stats.snapshot().pieces, home.piecesPlaced);
});
